"""
Scholarship Scraper — Buddy4Study + National Scholarship Portal (NSP)
=====================================================================
  • Buddy4Study  → Playwright scroll on /article/category/scholarship
...
() => {
    ...
    // Fallback: grab all scholarship article links directly
    if (cards.length === 0) {
...
def _classify_nsp(name: str) -> str:
    """Classify a scheme as scholarship / scholarship / fellowship / other."""
    n = name.lower()
...
def scrape_nsp() -> list:
    ...
    # ── Approach 2: Search for "scholarship" specifically ───────────────────
    # for keyword in ["scholarship", "scholar", "merit", "grant"]:
    #     log(f"  Searching NSP for keyword='{keyword}'…")
    #     resp = nsp_get(
    #         NSP_SEARCH,
...
async def main():
    ...
    csv_path  = OUTPUT_DIR / f"scholarships_{ts}.csv"
    json_path = OUTPUT_DIR / f"scholarships_{ts}.json"

    log("╔══════════════════════════════════════════════════════════╗")
    log("║  Scholarship Scraper  — Buddy4Study + NSP                 ║")
    log("╚══════════════════════════════════════════════════════════╝")
    log("")

    all_records: list = []

    # ── Step 1: NSP via requests (no browser needed, fast) ─────────────────
    try:
        nsp_records = scrape_nsp()
        # Enrich with detail pages (scholarship-category only to save time)
        scholarship_nsp = [r for r in nsp_records if r["category"] == "scholarship"]
        other_nsp  = [r for r in nsp_records if r["category"] != "scholarship"]

        scholarship_nsp = enrich_nsp(scholarship_nsp, max_pages=30)

        all_records = scholarship_nsp + other_nsp
        save(all_records, csv_path, json_path, "(after NSP)")
    except Exception as exc:
        log(f"NSP scraper error: {exc}")

    # ── Step 2: Buddy4Study via Playwright scroll ───────────────────────────
    try:
        b4s_records = await scrape_buddy4study(csv_path, json_path)
        # Deduplicate against existing
        existing_urls = {r["url"] for r in all_records}
        new_b4s = [r for r in b4s_records if r["url"] not in existing_urls]
        log(f"  Buddy4Study: {len(b4s_records)} total, {len(new_b4s)} new after dedup")

        # Enrich article pages for richer metadata
        new_b4s = enrich_buddy4study(new_b4s, max_pages=60)
    ...
    scholarship_n = sum(1 for r in all_records if r["category"] == "scholarship")
    ...
    log(f"║      Scholarships  : {scholarship_n}")
